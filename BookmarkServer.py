...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            ...
            self.end_headers()
            # List the known associations in the form for every key returned by the memory dictionary.
    ...
    def do_POST(self):
        ...
        # Check that the user submitted the form fields by using the input field names.
        ...
        if CheckURI(longuri):
            ...
            # Didn't successfully fetch the long URI, because it doesn't actually exist.
            self.send_response(404)
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
